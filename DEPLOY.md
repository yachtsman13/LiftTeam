...
Можно добавить ещё одну строку — `LABEL_BASE_URL`:

```ini
LABEL_BASE_URL=http://192.168.1.50
```

Она задаёт адрес, который попадает в QR-коды на этикетках деталей и ячеек.
Без неё берётся адрес, по которому открыта страница печати. Разница видна
в одном случае: если этикетку печатают, зайдя через Tailscale, а сканируют
телефоном в офисной сети, то в коде окажется адрес Tailscale, и телефон
без VPN его не откроет. Указав здесь локальный IP, вы получите этикетки,
которые читаются в офисе всегда. Уже напечатанные этикетки при этом
не меняются — параметр действует на новые.

**Чем короче этот адрес, тем надёжнее сканируются этикетки.** Длина ссылки
определяет размер QR-кода: `http://192.168.1.50` даёт код на 25 модулей,
а `http://lifteam.hvost-koshki.ts.net` — на 29. На этикетке 43×25 мм это
разница между 3,3 и 2,8 точками принтера на модуль, то есть между уверенным
и пограничным считыванием. Указывайте IP без порта, если он свободен.

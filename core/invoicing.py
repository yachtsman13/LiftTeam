...
Чего здесь нет. Проверки подлинности вебхуков: приём уведомлений
об оплате — отдельная задача, для неё нужен публичный адрес по HTTPS,
которого у программы пока нет. Пустого метода-заглушки под неё тоже
нет намеренно — интерфейс должен описывать то, что работает.

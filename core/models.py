...
def format_power(value):
    """Мощность в кВт текстом: «7,5», не «7.50», и «11», не «11,00».

    Два исправления сразу, тем же приёмом, что и `format_spec` ниже
    (`normalize()`, а после него — обратно из вида «3E+1» в обычную
    запись): хвостовые нули убираются — DecimalField хранит ровно
    столько знаков, сколько ввели или сколько задал шаг формы, а лишний
    ноль в счёте читается как точность, которой не было; и точка меняется
    на запятую — по-русски дробь пишут через неё.

    Не для полей ввода: там HTML5-виджет `<input type="number">`,
    и разделитель там обязан остаться точкой. В шаблонах (`{{ power_kw }}`)
    Django сам меняет точку на запятую по локали `ru-ru`, но хвостовые
    нули не убирает — там, где число не проходит через эту функцию,
    его выводит фильтр `power` (`core/templatetags/lifteam.py`), а не
    голая переменная.
    """
...
def plural_genitive(word):
    ...
    Тип бывает из нескольких слов, и первое слово в них — не всегда
    считаемый предмет. Два разных случая, и путать их нельзя:

    - «Диод Шоттки», «Транзистор MOSFET» — существительное, а за ним
      уточнение, которое в русском не склоняется вместе с ним:
      «диодов Шоттки», а не «диодов Шотткиов»;
    - «Линейный стабилизатор», «Керамический конденсатор» — впереди
      **прилагательное**, и предмет идёт следом. Склонять надо оба,
      согласованно: «линейных стабилизаторов».

    До v2.122.1 правило было одно — «склоняется первое слово», — и второй
    случай выходил кашей: «Набор линейныев стабилизатор». На этикетке
    ячейки это видно каждый день, а типов «прилагательное + предмет»
    в каталоге больше, чем эпонимов.

    Прилагательное узнаётся по окончанию (-ый/-ий/-ой/-ая/-яя) и **только
    когда за ним что-то стоит**: одиночный тип — почти наверняка
    существительное, и «Алюминий» не должен превратиться в «Алюминих»
    из-за похожего хвоста. Разбора фразы по частям речи в программе нет
    и не будет: словарь ради подписи на наклейке — не та цена.
    """
    phrase = (word or '').strip()
    if not phrase:
        return ''
    words = phrase.split()
    # Ведущие прилагательные — все, сколько их есть, но никогда последнее
    # слово: за ним уже нет предмета, к которому оно относится
    index = 0
    while index < len(words) - 1 and _looks_like_adjective(words[index]):
        words[index] = _decline_adjective(words[index])
        index += 1
    words[index] = _decline_first_word(words[index])
    return ' '.join(words)


# Окончания прилагательных в именительном падеже единственного числа.
# Мужской род — основной случай («линейный», «керамический»), женский
# добавлен той же строкой: «Керамическая плата» пишут так же охотно
ADJECTIVE_ENDINGS = ('ый', 'ий', 'ой', 'ая', 'яя')
# После шипящих и заднеязычных пишется «и», а не «ы»: «больших»,
# «керамических», но «линейных»
HUSHING = 'гкхжчшщ'


def _looks_like_adjective(word):
    return len(word) > 3 and word[-2:].lower() in ADJECTIVE_ENDINGS


def _decline_adjective(word):
    """«линейный» → «линейных», «керамический» → «керамических»."""
    stem, ending = word[:-2], word[-2:].lower()
    if ending in ('ий', 'яя'):       # синий → синих, летняя → летних
        return stem + 'их'
    return stem + ('их' if stem[-1:].lower() in HUSHING else 'ых')


def _decline_first_word(word):
    """Склонение одного слова — ядро `plural_genitive`, без разбора фразы."""
...
class Organization(models.Model):
    ...
    # Внутренний идентификатор участника ЭДО у Диадока — не ИНН и не
    # угадывается: узнаётся запросом GetMyOrganizations после регистрации
    # юрлица в Диадоке. Без него УПД сформировать нельзя (core/diadoc.py)
    diadoc_fns_participant_id = models.CharField(
        'Идентификатор участника ЭДО (Диадок)', max_length=100, blank=True,
        help_text='Для формирования УПД. Узнаётся через GetMyOrganizations '
                  'в личном кабинете интегратора Диадока, вручную не придумывается.'
    )
...
class Client(models.Model):
    ...
    # Тот же идентификатор, что у Organization.diadoc_fns_participant_id,
    # но у заказчика — доступен только после того, как он примет
    # приглашение к обмену документами в Диадоке (см. core/diadoc.py)
    diadoc_fns_participant_id = models.CharField(
        'Идентификатор участника ЭДО (Диадок)', max_length=100, blank=True,
        help_text='Для формирования УПД. Узнаётся через GetCounteragents '
...
class FaultType(models.Model):
    ...
    # Что делают, чтобы это устранить. Отдельно от `description`, и это
    # не педантизм: описание отвечает «что нашли», работы — «что сделали»,
    # у них даже время глагола разное. Акт выполненных работ с фразой
    # «конденсаторы потеряли ёмкость» читается как отчёт о том, что ничего
    # не делали.
    #
    # В документы попадает не сам этот текст, а то, что мастер принял:
    # кнопка на странице единицы подставляет его в поле работ, дальше он
    # правит словами. Акт подписывает он, и вписывать за него программа
    # не вправе — то же правило, что у цены из прайса.
    work_description = models.TextField(
        'Типовые выполненные работы', blank=True,
        help_text='Что делают, чтобы это устранить: «Заменены '
                  'электролитические конденсаторы в цепи питания». '
                  'Подставляется кнопкой в заказе, в поле выполненных работ.'
    )
    ...
    def work_text(self):
        ...
        Пусто — подставлять нечего, и короткое название вместо работ
        не идёт: оно не для документов, а для списков.
        """
        return self.work_description.strip()
...
class TechCardStep(models.Model):
    ...
    # Снимок — единственный файл, который программа хранит у себя.
    # Схемы и инструкции по-прежнему ссылками (см. EquipmentMaterial):
    # их правят на Диске, и копия разошлась бы с оригиналом. Снимок шага
    # никто не правит — он показывает, как это выглядит на столе.
    #
    # Перед сохранением уменьшается (см. core/images.shrink_photo):
    # с телефона приезжает четыре мегабайта, а программа живёт на карте
    # памяти и выгружает копии по домашнему каналу.
    #
    # Файл при удалении шага НЕ стирается: он весит около сотни килобайт,
    # а стирание по случайному удалению строки — способ потерять снимок,
    # который переснять уже негде.
    image = models.ImageField(
        'Снимок', upload_to='tech_cards/%Y/%m/', blank=True,
...
class RepairOrder(models.Model):
    ...
    def assign_equipment_owners(self):
        ...
        владельца не перезаписываем никогда** — так решил владелец
        программы: прибор нередко приезжает от другого обслуживающего
        предприятия, и это не значит, что он сменил хозяина. У кого он
    ...
    def quote_rows(self):
        ...
        for order_equipment in self.order_equipments.select_related('equipment__model').order_by('id'):
            rows.append({
                ...
                'equipment_name': order_equipment.quote_equipment_name,
...
class RepairOrderEquipment(models.Model):
    ...
    # Наименование изделия в коммерческом предложении. Пусто — печатается
    # обозначение из справочника («Преобразователь частоты Emotron-2.0-1.1»).
    # Правится руками, потому что исполнение значит разное у разных
    # изделий: у преобразователей частоты версии отличаются мощностью,
    # то есть и ценой ремонта, а у приводов дверей на цену почти не влияют.
    # Что из этого важно заказчику в этом предложении, решает мастер.
    quote_designation = models.CharField(
        'Наименование в предложении', max_length=255, blank=True,
    ...
    def typical_work_lines(self):
        """Типовые работы выбранных неисправностей — по строке на каждую.

        Порядок тот же, что у неисправностей: мастер читает их сверху
        вниз и так же читает получившийся текст. Пустые пропускаются
        молча — подставлять нечего.

        Это **заготовка**, а не текст документа: в акт идёт то, что стоит
        в `work_performed`, то есть то, что мастер принял и поправил.
        Собирать акт из неисправностей самой программе нельзя — их
        выбирают при дефектации, до ремонта, и акт оказался бы готов
        по неразобранному прибору.
        """
        return [text for text in (fault.work_text for fault in self.faults.all()) if text]

    @property
    def typical_work_json(self):
        """То же самое для разметки: кнопка подстановки читает строки
        из атрибута, а не из отдельного запроса.

        JSON, а не перечисление через запятую: в описании работ бывают
        и запятые, и кавычки, и переводы строк.
        """
        return json.dumps(self.typical_work_lines, ensure_ascii=False)
    ...
    # Четвёртым в строке стоит **раздел страницы единицы**, в котором этот
    # пункт закрывают (с v2.95.0). Он здесь, а не в шаблоне, по той же
    # причине, что и всё остальное в этом списке: страница единицы красит
    # разделы по готовности, и «какой пункт к какому разделу относится»
    # обязано быть записано один раз. Разъедься эти два места — раздел
    # горел бы недоделанным при заполненном поле.
    READINESS_CHECKS = (
    ...
    # Разделы страницы единицы — по ходу работы, тем же порядком, что
    # и сами проверки. Названия нужны там, где о разделе говорят словами
    # (значок в шапке карточки), и берутся отсюда, а не пишутся в шаблоне.
    READINESS_SECTIONS = (
        ('diagnosis', 'Приём и диагностика'),
        ('repair', 'Ремонт'),
        ('parts', 'Детали'),
    )
    ...
    def readiness(self):
        ...
        Возвращает список словарей `{code, label, hint, section, done, url}`
        ...
        return [
            ...
            for code, label, hint, section in self.READINESS_CHECKS
            if code not in skip
        ]

    @property
    def readiness_sections(self):
        """Готовность по разделам страницы единицы (с v2.95.0).

        `{'diagnosis': {'done': True, 'pending': [], 'label': '…'}, …}` —
        страница красит этим сами разделы, вместо отдельного списка
        внизу. Список внизу отвечал на вопрос «что осталось» в стороне
        от полей, которые это закрывают: мастер читал пункт, шёл в раздел
        и там заново искал глазами, чего не хватает.

        Раздел без своих проверок (такое бывает у гарантийного ремонта,
        где пункт стоимости выпадает вовсе) считается сделанным: гореть
        ему не от чего.
        """
        checks = self.readiness()
        sections = {}
        for code, label in self.READINESS_SECTIONS:
            mine = [check for check in checks if check['section'] == code]
            sections[code] = {
                'label': label,
                'checks': mine,
                'pending': [check for check in mine if not check['done']],
                'done': all(check['done'] for check in mine),
            }
        return sections
    ...
    def has_pending_planned_parts(self):
        """Есть ли деталь, намеченная по ремонту, но ещё не взятая со склада.

        То же условие, что у пункта готовности `planned_parts` — здесь
        нужно отдельно, чтобы покрасить кнопку «Детали» в списке единиц
        заказа, не пересчитывая весь список проверок ради одного пункта.
        """
        return any(detail.is_planned for detail in self.details.all())
    ...
    @property
    def quote_equipment_name(self):
        """Название изделия в предложении: правленное руками либо своё.

        Собирается одним местом, как и текст неисправности: заказчик
        не должен получить в одном документе одно название изделия,
        а в другом другое.

        По умолчанию — полное обозначение с родовым словом, как в акте
        дефектации: «Привод дверей БУАД-7-31.4». Предложение и акт
        читает один и тот же человек, и изделие в них обязано называться
        одинаково.
        """
        return self.quote_designation.strip() or self.equipment.full_designation
...
class SparePart(models.Model):
    ...
    description = models.TextField('Описание', blank=True)
    # Даташит — вторая (и пока последняя) вещь, которую программа хранит
    # у себя файлом, тем же приёмом, что снимок к шагу техкарты
    # (TechCardStep.image): в отличие от материалов модели оборудования
    # (EquipmentMaterial, ссылка на Диск) даташит — не то, что мы сами
    # правим, а завершённый документ производителя, и разойтись копии
    # с оригиналом там неоткуда. Своя копия здесь ценнее ссылки: ссылки
    # на сайтах производителей и дистрибьюторов протухают через пару лет,
    # а найти нужный даташит второй раз — та самая работа, которую поле
    # избавляет делать заново. Как и снимок шага, при замене или удалении
    # детали файл не стирается — весит немного, а стереть по ошибке
    # и потерять — хуже, чем накопить лишний.
    datasheet = models.FileField(
        'Даташит', upload_to='datasheets/%Y/%m/', blank=True,
    ...
    def label_text(self):
        ...
        Оба сразу не печатаем никогда — на 26 мм ширины не разместить и то,
        и другое, не потеряв в читаемости обоих.

        Пробел между числом и следующей за ним единицей — неразрывный,
        тем же приёмом, что у сумм (`format_amount`): текст свободный,
        его набирает человек, и «10 Ом» с обычным пробелом на переносе
        строки стало бы «10» на одной строке и «Ом» на другой.
        """
        text = (self.description or '').strip()
        if not text:
            name = (self.name or '').strip()
            text = '' if name == self.part_number else name
        return re.sub(r'(?<=\d) (?=[^\W\d_])', ' ', text)

...
def plural_genitive(word):
    ...
    Тип бывает из нескольких слов («Диод Шоттки», «Транзистор MOSFET») —
    склоняется только **первое**: это и есть считаемый предмет, а дальше
    идёт уточнение, которое в русском не склоняется вместе с ним
    («диодов Шоттки», а не «диодов Шотткиов»). Раздельного разбора фразы
    на существительное и определение в программе нет и не будет — форма
    выводится тем же простым правилом, что и одно слово, просто
    применённым только к первому.
    """
    word = (word or '').strip()
    if not word:
        return ''
    head, sep, rest = word.partition(' ')
    return _decline_first_word(head) + sep + rest

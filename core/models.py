...
def order_status_css(status):
    """Цвет статуса заказа — один на всю программу (с v2.99.0).
...
# ==================== ДОЛЖНОСТИ И ПРАВА ====================
#
# До v2.99.0 прав как таковых не было: у сотрудника стояла одна из четырёх
...
class EmployeeManager(BaseUserManager):
    ...
    def create_superuser(self, username, full_name, password=None, **extra_fields):
        ...
        # прав в программе один, и им остаётся должность (с v2.99.0)
...
class Employee(AbstractBaseUser, PermissionsMixin):
    ...
    Права с v2.99.0 даёт **должность** (`Position`), а не зашитая в код
...
class ClientContact(models.Model):
    """Контактное лицо заказчика — с v2.99.0.
...
class FaultType(models.Model):
    ...
    # стоит вентилятор), и до v2.99.0 отбора по этому вовсе не было —
...
class RepairOrder(models.Model):
    ...
    STATUS_CHOICES = [
        ...
        # 2026-08-31 — с v2.99.0. Терминальный статус, как «Отгружен»
...
class RepairOrderEquipment(models.Model):
    ...
    # Кто чинил эту единицу — с v2.99.0. Не «ответственный по заказу»
    ...
    @property
    def earnings(self):
        """Заработок исполнителя по этой единице — с v2.99.0.

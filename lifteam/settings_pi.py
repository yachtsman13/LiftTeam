...
#
# По умолчанию — имя Pi в Tailscale (MagicDNS), а НЕ его адрес 100.x.
# Разница принципиальная: этикетка — бумажка, адрес в ней впечатан навсегда.
# Адрес 100.x закреплён за узлом, но меняется, если узел удалить из тайнета
# и завести заново — например, при переустановке системы. Все наклеенные
# к тому моменту этикетки после этого мертвы: перепечатать их можно,
# отклеить с сотни пакетов — нет. Имя же переживает смену адреса.
#
# Размер кода от этого не страдает, вопреки опасению: и адрес, и имя дают
# один и тот же QR. Границы версий по длине ссылки (уровень коррекции M):
#   до 26 символов — 25 модулей
#   27–42          — 29 модулей
#   43 и больше    — 33 модуля
# «http://100.108.92.92/o/1234» — 27 символов, уже 29 модулей;
# «http://lifteam.taile9b605.ts.net/o/1234» — 39, те же 29. Запас до 42
# символов покрывает семизначные номера, то есть не кончится никогда.
# Проверка длины есть на самих страницах печати (QR_MAX_CHARS в views).
#
# Адрес Pi один и тот же в офисе, дома и с телефона в дороге, поэтому
# этикетка читается одинаково независимо от того, кто и откуда её печатал.
# Локальный IP так не умеет: напечатанная через Tailscale этикетка получала
# бы адрес, недоступный из офиса, и наоборот.
#
...
LABEL_BASE_URL = os.getenv('LABEL_BASE_URL', 'http://lifteam.taile9b605.ts.net')

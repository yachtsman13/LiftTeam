...
v2.51.0 — standalone (SQLite) / Docker (PostgreSQL + Redis + Nginx)
"""
import os
...
#
# Приём выключен по умолчанию у каждого банка отдельно, и это не
# перестраховка: работающей проверки подписи в программе НЕТ ни у одного
# банка — схемы подписи описаны в документации банков, а она недоступна
# (подробности — в шапке core/webhooks.py). Включённый приём с ненаписанной
# проверкой означал бы адрес в интернете, по которому любой желающий
# отмечает чужие счета оплаченными. Пока проверка не написана, включённый
# приём всё равно отвечает отказом на каждый запрос — но включать его
# всё равно не надо.
...
# Секрет, которым проверяется подпись уведомления. Свой на каждый банк:
# как и у токенов, общий секрет рано или поздно означал бы проверку
# уведомления одного банка ключом другого. Чем именно его заполнять,
# станет известно из документации банка вместе со схемой подписи.
WEBHOOKS_TBANK_SECRET = os.getenv('WEBHOOKS_TBANK_SECRET', '')
WEBHOOKS_TOCHKA_SECRET = os.getenv('WEBHOOKS_TOCHKA_SECRET', '')

# Список адресов, с которых принимаются уведомления. Пустой — здесь
# не фильтруем. Так и задумано: за обратным прокси приложение видит адрес
# прокси, а не банка, поэтому отсекать по адресу правильнее в nginx.
# Адреса выдаёт банк — выдуманный список либо не защищает, либо отсекает
